#!/usr/bin/env python3
"""Build a distributable zip of the EDRavenColonialAgent EDMC plugin.

Auto-detects the plugin subfolder (the one containing ``load.py``), reads its
``__version__``, and writes ``dist/<PluginFolder>-v<version>.zip`` with the plugin
folder as the top-level entry, so users extract it straight into EDMC's ``plugins/``
directory. Python caches are excluded.
